...
AGENT_TOOLS: list[dict[str, object]] = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "由加先生の過去の実際の回答例を意味検索する（Embedding検索）。"
                "受講生の相談・体験報告・メソッド質問に対する由加先生の口調や"
                "アドバイスの参考として使用する。consultation, method_question, "
                "experience_report の意図で特に有効。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
- empathy (consultation) → 【必須】search_faq(query, source="auto") を必ず呼ぶ + lookup_terms(query) + search_golden_answers(query)
- encouragement (experience_report) → lookup_terms(query) で用語の背景を確認 + search_golden_answers(query)
- detailed_feedback (method_question) → 【必須】search_faq(query, source="auto") を必ず呼ぶ + lookup_terms(query) + search_golden_answers(query)
- brief (greeting) → 追加ツール不要

## search_golden_answers の活用ルール
- consultation / method_question / experience_report では必ず search_golden_answers を呼ぶ
- 由加先生の過去の実際の回答が返ってくるので、そのトーン・アドバイスの方向性を参考にする
- 【重要】golden_answers の回答をそのままコピーしない。あくまで口調やアプローチの参考にする
- 背景（context）が似ている場合は、由加先生がどう対応したかを踏まえて回答する
- golden_answers に該当がない場合でも、FAQデータやスピ用語で回答を構成する
...
### 最優先ルール: FAQデータの回答をそのまま使う
- 【絶対厳守】search_faq の結果にある回答文をそのまま使うこと。GPTの知識で要約・言い換え・解釈をしない
- 元データにない情報や表現を追加しない（「主に自己の内面を探求する」のようなGPTの解釈は禁止）
- 元データの回答で「〜でなくても大丈夫」「〜は不要」「〜は自由参加」のような安心させる表現がある場合、必ず含める
- FAQデータで完結する質問には「個別相談LINEへどうぞ」「サポート担当者にお繋ぎします」を追加しない。FAQの回答に誘導先が書いてある場合のみ含める
- 3行以内の制約は「要点を絞る」意味であり「FAQの具体情報を省略する」意味ではない

### FAQ質問（brief）の回答構造
1行目: FAQデータの回答文の核心をそのまま伝える
2行目: 補足情報（FAQデータにある場合のみ）
3行目: 誘導先（FAQデータに書いてある場合のみ。FAQで完結するなら不要）

### 複数の項目を比較する質問への対応
「〜と〜の違いは？」「〜の関係は？」のように複数項目の比較質問では、FAQデータに各項目の説明がある場合、箇条書きで各項目の違いを並べること。GPTの解釈で要約しない。
...
def _tool_search_golden_answers(args: dict[str, object]) -> dict[str, object]:
    """ゴールデンアンサー検索ツール（Embedding検索）。"""
    query = str(args["query"])
    results = search_golden(query, top_k=3)
    if not results:
        return {
            "examples": [],
        ...
        "examples": [
            {
                ...
                "context": (g.get("context", "") or "")[:150],  # 長すぎ防止
                "question": g["question"],
                "answer": g["answer"],
                "intent": g.get("intent", ""),
